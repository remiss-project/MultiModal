...
jsonl_files     = ['remiss_data_complete/barcelona_2019_search-1.media.jsonl', 'remiss_data_complete/generales_2019_search-1.media.jsonl', 'remiss_data_complete/generalitat_2021_search-1.media.jsonl', 'remiss_data_complete/MENA_Agressions.media.jsonl', 'remiss_data_complete/MENA_Ajudes.media.jsonl', 'remiss_data_complete/Openarms.media.jsonl']
...
current_local_dir_path='client_runtime_remiss_docker/'

client = Client("https://158.109.8.130:7860/", ssl_verify=False,auth=("remiss", "t6y7u8?"))
placeholder_path= os.path.join(current_local_dir_path, 'no-results.png')
...
#user_labelled={}
user_labelled=np.load(os.path.join(current_local_dir_path,'outputs/user_input_labels.npz'),allow_pickle=True)
...
def user_input_labels_PRISTINE():
    ...
    np.savez(os.path.join(current_local_dir_path,'outputs/user_input_labels.npz'),user_labelled=user_labelled)
...
def remiss_sample_already_stats(jidx):
    ...
    data_path       = os.path.join( current_local_dir_path,'remiss_data_complete/', split_type) 
    vl_dict         = np.load(os.path.join(data_path,'remiss_filter_marked.npz'), allow_pickle=True)
    vl_dict         = vl_dict[vl_dict.files[0]]
    vl_dict         = vl_dict.flat[0] 
    
    output_base_folder =os.path.join(current_local_dir_path, 'outputs/',  split_type) 
...
def load_remiss_sample_already(jidx):
    # Assuming test_samples is defined elsewhere in your code
    #
    jsonl_file      = current_local_dir_path+jsonl_files[jidx]
    ...
    while True:
        ...
        output_folder = f'{current_local_dir_path}outputs/{split_type}/{current_index}'
...
def load_from_folder(givenfolder=None):
    ...
    if givenfolder:
       ...
       output_folder = f'{current_local_dir_path}outputs/{split_type}/{sample_idx}/'
...
def save_outputs_to_folder(new_CDG0, new_VDG0, new_TDG0, new_dom0, xv_cat0, xv_text0, xvs0, xv_img0, xv_gs0, xt,xt_scr, xt_gs0, new_CDG1, new_VDG1, new_TDG1, new_dom1, xv_cat1, xv_text1, xvs1, xv_img1, xv_gs1, xt_gs1, text_claim, img_claim, found_flag, ci, t_sug, old_t_sug):
    global sample_idx
    output_folder =  os.path.join(current_local_dir_path,'outputs/',split_type,str(sample_idx))
...
def remiss_sample(jidx):
    print('trying to load a remiss_sample............................')
    #logger.debug('<debug> 3')

    jsonl_file      = os.path.join(current_local_dir_path,jsonl_files[jidx])
    ...
    text_input = cd['text']
    media = cd.get('media', [])
    v_claim_p = os.path.join(data_path, 'claim_images/','claim_'+str(cd['id'])+'_img.jpg')
    img_input = Image.open(v_claim_p)
    print('success_remiss_sample')
...
def load_claim_debug():
    #logger.info("Loading claim")
    #logger.debug(f"Inputs: {img_input}, {text_input}, {node_thres_input_level}, {edge_thres_input_level}, {vis_sim_thres_input_level}")
    # Simulate processing

    #logger.debug('<debug> load_claim')
    #logger.debug(img_input)
    return ['no_evidence']
    #return 'x','x','not found','no_evidence',420 
def load_claim(img_input, text_input, node_thres_input_level, edge_thres_input_level, vis_sim_thres_input_level):
    ...
    try:
       ...
       #logger.debug('success: claim image loaded')
    except:
       #logger.debug('fail: claim image load issue')
       pdb.set_trace()   
...
with gr.Blocks(theme=gr.themes.Default(spacing_size="sm", radius_size="none", text_size="lg")) as app:
    ...
    save_button.click(save_outputs, inputs=[new_CDG0, new_VDG0, new_TDG0, new_dom0, xv_cat0, xv_text0, xvs0, xv_img0, xv_gs0,xt,xt_scr,xt_gs0, new_CDG1, new_VDG1, new_TDG1, new_dom1, xv_cat1, xv_text1, xvs1, xv_img1, xv_gs1, xt_gs1, text_input, img_input, found_flag, ci, t_sug, old_t_sug], outputs=[])
