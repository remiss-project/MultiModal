...
#import fasttext
import pdb
from utils.config import save_folder_path,split_type,sub_split,rev_ser_api,rev_gse_id
...
my_api_key = rev_ser_api
my_cse_id =  rev_gse_id

#PRETRAINED_MODEL_PATH = 'lid.176.bin'
#model = fasttext.load_model(PRETRAINED_MODEL_PATH)

full_save_path = os.path.join(save_folder_path,split_type,'inverse_search',sub_split)
        ...
        if page.full_matching_images:
            ...
            for image_url in page.partial_matching_images:
                try:
                    ...
                    print("Error in getting captions - id in clip:"+ str(id_in_clip))
                    continue
                if not 'caption' in new_entry.keys(): 
                    continue 
                else:
...
def reverse_search_and_save_one_query(image_path, id_in_clip):
...
def reverse_getdatafromjson(jsonfile):
    f = open(jsonfile)
    data = json.load(f)
    
    ent= data['entities']
    ent_scr= data['entities_scores'] 
    bgl= data['best_guess_lbl']
    data_mc= data['all_fully_matched_captions']
    data_pc= data['all_partially_matched_captions']
    textev=[]
   
    ##
    ret_cap='no_text_evidence'
    for datum in data_mc:
        try:
           ret_cap=str ( datum['caption'] ) + datum['title']  ## editidex, bc caps are fked for remiss,always using titles
        except:
           ret_cap='no_text_evidence'   
        textev.append(ret_cap)
    
    
    ## no partial match for remiss    
    '''for datum in data_pc:
        try:
           ret_cap=str ( datum['caption'] ) + datum['title']  ## editidex, bc caps are fked for remiss,always using titles
        except:
           ret_cap='' 
    ...
    #pdb.set_trace()  
    if len(textev)==0:
           textev=['no_text_evidence']
    
       
    return ent,ent_scr,bgl,textev
